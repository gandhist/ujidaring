...
<div class="container-fluid">
  <div class="row">
    ...
    <div class="col-lg-6">
      ...
      <p>Waktu Ujian sampai: {{ \Carbon\Carbon::parse($modul_today->akhir_pre_quiz)->isoFormat('DD MMMM YYYY HH:mm:SS') }}</p>
...
$(document).ready(function () {
  ...
  $('body').on('click', '.pagination a', function(e) {
    ...
    var eventTime = "{{ strtotime($modul_today->akhir_pre_quiz) }}"; // Timestamp - Sun, 21 Apr 2013 12:30:00 GMT
...
function storeDataPar(id, val){
    var url = "{{ url('peserta/quis/pre/save_parsial') }}";
    $.ajaxSetup({
      headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
      }
...
function storeDataParEs(id, val){
    var url = "{{ url('peserta/quis/pre/tm/save_parsial') }}";
    $.ajaxSetup({
      headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
      }
...
function storeData(){
    var formData = new FormData($('#formAdd')[0]);
    var url = "{{ url('peserta/quis/pre/save') }}";
    $.ajaxSetup({
      headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
      }
