                    ...
                    <table id="custom-table" class="table table-striped table-bordered dataTable customTable">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                @php
                                $cek = DB::table('uji_jawaban_evaluasi')->select('tanggal','id_instruktur','id_jadwal')->where('id','=',$id_jaw_evaluasi)->where('deleted_at',null)->first();
                                $tanggal = $cek->tanggal;
                                $id_jadwal = $cek->id_jadwal;
                                $idinstruktur = $cek->id_instruktur;
                                $jaw_eval = DB::table('uji_jawaban_evaluasi')->where('tanggal','=',$tanggal)->where('id_jadwal',$id_jadwal)->where('id_peserta',$key->id)->where('id_instruktur',$idinstruktur)->where('deleted_at',null)->whereNotNull('nilai')->count();
                                @endphp
                                @if($jaw_eval>0)
